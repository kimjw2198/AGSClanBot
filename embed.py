...
async def on_ready():
    ...
    game = discord.Streaming(name="AGS Clan Bot", url='https://www.twitch.tv/AGS-ClaN')
...
async def on_message(message):
    ...
    if message.content.startswith("/임베드"):
        if message.author.guild_permissions.manage_messages:
            ...
            embed.set_author(name="CLAN AGS (Argos)")
    ...
    if message.content.startswith("/경고"):
        if message.author.guild_permissions.manage_messages:
            ...
            embed.set_author(name="CLAN AGS (Argos) 제재")
            await client.get_channel(819967923984269322).send(embed=embed)
    ...
    if message.content.startswith("/청소"):
        ...
        embed.set_author(name="CLAN AGS (Argos) 메시지 삭제")
        await message.channel.send(embed=embed)
        time.sleep(10)
    ...
    if message.content.startswith("/공지등록"):
        if message.author.guild_permissions.manage_messages:
            ...
            embed.set_author(name="CLAN AGS (Argos) 공지사항")
            embed.set_footer(text="공지사항 숙지 부탁드립니다.")

            await client.get_channel(819889834024894504).send(embed=embed)
